...
const (
    ...
    authPath  = "/authorize"
    tokenPath = "/token"
    //oauth parameters for this application
    clientId = ""
    secret   = ""
    redirect = "http://localhost:8085/oauth2/callback"
    //api user path
    mePath = "/api/v1/users/me"
)
var (
    authUrl, _ = url.Parse("https://signin.mypurecloud.com")
    apiUrl, _  = url.Parse("https://public-api.us-east-1.mypurecloud.com")
)
...
func main() {
    ...
    http.HandleFunc("/oauth2/callback", callback)
    log.Panic(http.ListenAndServe(":8085", nil))
}
